...
public class HumanoidFootIK2D : MonoBehaviour {
	...
	protected void CalculateFoot(Transform foot, ref float footWeight, string weightParamName, AvatarIKGoal footGoal, ref Vector3 lastIKPos)
	{
		...
		//2D Physics
		RaycastHit2D hitInfo2D = Physics2D.Raycast(checkPos, Vector2.down, checkDistance, ~(ignoreOnRaycast));
		if(hitInfo2D.collider != null)
		{
			...
			{
				raycastPosYs.Push(hitInfo2D.point.y);
			}

			animator.SetIKPositionWeight(footGoal, footWeight);

			Vector3 ikPos = new Vector3(hitInfo2D.point.x, hitInfo2D.point.y, foot.position.z);
			...
			Vector3 toeRollLookAngle = Vector3.Cross(-foot.right, hitInfo2D.normal); 
